...
CREATE TABLE `sys_user_html` (
  ...
  `pageid` varchar(200) NOT NULL COMMENT '页面ID',
  `domid` varchar(100) NOT NULL COMMENT '页面中元素ID',
  `modid` varchar(100) DEFAULT NULL COMMENT '模块ID',
  `createdt` datetime NOT NULL COMMENT '创建时间',
  `updatedt` datetime NOT NULL COMMENT '更新时间',
  `optdata` varchar(200) DEFAULT NULL COMMENT '界面配置数据',
  `typecd` varchar(100) DEFAULT NULL COMMENT '类别编码',
  PRIMARY KEY (`pid`,`pageid`,`domid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='BI图表-用户与界面关系';
