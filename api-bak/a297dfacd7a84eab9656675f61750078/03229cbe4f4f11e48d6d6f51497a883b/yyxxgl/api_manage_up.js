...
if (dbType == "01"){
sqls = "select appid,moduleid,apiid,content from sys_apis a left join sys_api_content b on a.contentid = b.contentid and a.status='1' where concat(appid,'--',moduleid,'--',apiid) in ( "+inidstr+" ) ";
}else if(dbType == "02"){
   sqls = "select appid,moduleid,apiid,content from sys_apis a left join sys_api_content b on a.contentid = b.contentid and a.status='1' where appid + '--'+ moduleid+ '--'+ apiid in ( "+inidstr+" ) "; 
}else if(dbType == "03"){
  sqls = "select appid,moduleid,apiid,content from sys_apis a left join sys_api_content b on a.contentid = b.contentid and a.status='1' where appid|| '--'|| moduleid|| '--'|| apiid  in ( "+inidstr+" ) ";  
}
if(""!=inidstr){
