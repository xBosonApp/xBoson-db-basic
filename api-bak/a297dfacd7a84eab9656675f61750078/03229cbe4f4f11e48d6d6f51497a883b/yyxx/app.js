...
//staa1
  ...
  var ret=[];
  // 验证 APPID 是否已经存在
  var sqlAppCnt = "select count(*) cnt from "+org+".sys_apps where appid = ? ";
  ...
  if (type == "0") {  // 创建操作
    ...
    sqlApp = "insert into "+org+".sys_apps (appid, appnm, about, appflag, status, createdt, updatedt) values (?, ?, ?, ?, ?, ?, ?)";
    ...
    // 根据请求数据更新 apps 数据
    sqlApp = "update "+org+".sys_apps set ";
    if (appNm != null) {
      sqlApp = sqlApp + "appnm = ?, ";
      @param.add(appNm);
    }
