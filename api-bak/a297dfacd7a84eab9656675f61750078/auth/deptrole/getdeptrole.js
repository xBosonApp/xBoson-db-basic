...
sql=sql+"case when b.deptid is null then 'false' else 'true' end as checked";
sql=sql+" from sys_role a left join sys_dept_role b on a.roleid=b.roleid and b.deptid=? where a.orgid=? and (a.comm_flag != '1' or a.comm_flag is null) and a.role_type = '01'";
...
//获取机构应用角色
    ...
    if(sys.size(approle) == 0){
        sys.setRetData("0","","result","comm_role");
        return;
    }
    //非管理员仅列出他有的角色
...
//设置复选框状态
    var authtmp1 = "select roleid from sys_dept_role where deptid=? and status = '1'";
    var authcnt1 = sql.query(authtmp1, [deptid], "authresult1");
