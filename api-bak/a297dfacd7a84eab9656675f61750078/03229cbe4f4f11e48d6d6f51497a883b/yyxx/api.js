...
// http://localhost:8080/ds/ide/api?app=mdm&org=zr&s=d&openid=dean&type=0&appid=z&modid=z&apiid=zzz&nm=znm1&cmt=cmt&status=1
  ...
  var op_type = sys.request.optype;
  ...
  var sqlApiCnt = "select count(*) cnt from "+orgid+".sys_apps sys_apps, "+orgid+".sys_modules sys_modules, "+orgid+".sys_apis sys_apis where sys_apps.appid = sys_modules.appid and sys_apps.appid = sys_apis.appid and sys_apps.appflag = '0' and sys_modules.auflag = '0' and sys_apis.appid = ? and sys_apis.moduleid = ? and sys_apis.apiid = ?";
  ...
  if (type == "0") {  // 创建操作
    ...
    var sqlApi = "insert into "+orgid+".sys_apis (appid, moduleid, apiid, apinm, contentid, status, createdt, updatedt, op_type,help_info) values (?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
    ...
    var sqlApiContent = "insert into "+orgid+".sys_api_content (contentid, stability, pid, updatedt) values (?, '00', ?, ?)";
    ...
    sql.commit();
    //返回添加的数据
    var ret=[{"appid":appId,"moduleid":modId,"apiid":apiId,"apinm":apiNm,"contentid":contentId,"status":newApiStatus,"createdt":currentTime,"updatedt":currentTime,"appnm":apiNm,"someid":appId+"@"+modId+"@"+apiId,"isParent":false, "optype":op_type}];
    ...
    // 根据请求数据更新 apis 数据
    var param = [];
    var sqlApi = "update "+orgid+".sys_apis set ";
    if (apiNm != null) {
      sqlApi = sqlApi + "apinm = ?, ";
      @param.add(apiNm);
    }
    ...
    sql.commit();
    //查询api的稳定性状态
    var selectstability="select b.stability from "+orgid+".sys_apis a , "+orgid+".sys_api_content b where a.contentid=b.contentid and a.appid = ? and a.moduleid = ? and a.apiid = ?";
    ...
    var ret=[{"appid":appId,"moduleid":modId,"apiid":apiId,"apinm":apiNm,"updatedt":dt,"appnm":infonm+apiNm,"someid":appId+"@"+modId+"@"+apiId,"isParent":false, "optype":op_type}];
    sys.addRetData(ret,"result");
  }
  //更新角色api缓存
