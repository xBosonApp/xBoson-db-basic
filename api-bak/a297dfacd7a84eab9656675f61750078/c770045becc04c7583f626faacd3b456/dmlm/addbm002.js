...
if(modolnm==null || dstypecd==null || sqltype==null || jsondata_str==null || status==null || modolcd==null  || tablenm==null || did==null){
    sys.setRetData("1");
    return;
}
//检查主键是否存在
...
//验证modolcd以dstypecd开头，以sqltype结尾
if(sys.startWith(modolcd,dstypecd) && sys.endWith(modolcd,sqltype)){
}else{
    sys.setRetData("2","业务模型ID格式错误");
    return;
}
//调接口生成SQL语句
...
var params=[modolcd,modolnm,dstypecd,did,tablenm,sqltype,sqltext,sqlparams,jsondata_str,typecontent,isui,status,mark,dt,dt];

sql.update(ins,params);
// 为所有开发角色加上模型权限
var typecdArr = [modolcd];
