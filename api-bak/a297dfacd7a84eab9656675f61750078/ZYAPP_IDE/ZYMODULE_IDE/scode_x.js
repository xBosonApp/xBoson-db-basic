...
if (contentId) {
  ...
  var checkAuth = `Select 1
    ...
    Left join sys_role_api ua on rug.roleid = ua.roleid
    Left join sys_apis a on a.apiid = ua.apiid
    Left join sys_api_his_content h on h.contentid = a.contentid
   Where rug.status = 1 And u.status = 1 And ua.status = 1
     And u.pid = ?
     And h.hisid = ?`;
  if (sql.query(checkAuth, [sys.getUserPID(), hisId]) < 1) {
    return sys.setRetData(1100, '无权操作');
  }
} else {
  sys.setRetData("1");
...
if (contentId != null && contentId != "") {
  sqlApiContent = "select api.contentid,api.content, api.stability, api.updatecmt, api.updatedt, person.de0201039 person, api.zip from sys_api_content api left join mdm_personal_info person on api.pid = person.pid where contentid = ?";
  paramApiContent = [contentId];
} else if (hisId != null && hisId != "") {
  sqlApiContent = "select api.content, api.stability, api.updatecmt, person.de0201039 person, api.updatedt, api.zip from sys_api_his_content api left join mdm_personal_info person on api.pid = person.pid where hisid = ?";
...
if (cnt == 0) {
  ...
  var api = sys.result.content[0];
  api.content = se.decryptApi2(api.content, api.zip);
